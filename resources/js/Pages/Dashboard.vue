...
const props = defineProps(['response', 'errors']);

const form = reactive({
  search: null,
});

const searchResults = null; // Use reactive instead of ref

// Watch for changes in props.response and update searchResults accordingly
watch(() => props.response, (newSearchResults) => {
  searchResults = newSearchResults.Search;
});
